...
def get_strategy_capital_data():
    ...
    btc_holdings = find_number(
        [
            ...
            r"(?:BTC holdings|Bitcoin holdings)"
            r".{0,500}?"
            r"([0-9]{1,3}(?:,[0-9]{3})+)"
            r"\s*BTC",
...
def update_data_json():
    ...
    try:
        ...
        if (
            latest["btcHoldings"] <= 0
            or
            latest["fdso"] <= 0
        ):

            raise RuntimeError(
                "SEC returned invalid "
                "company data"
            )
        ...
        if (
            latest["debtUsdB"] < 0
            or
            latest["preferredUsdB"] < 0
            or
            latest["usdAssetsUsdB"] < 0
        ):
